...
public class PlayerMove3 : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D collider)
    {
        ...
        if (collider.gameObject.CompareTag("Win"))
        {
            if (coinCount == 50)
            {
                SceneManager.LoadScene("S3WinScreen");
            }
            else if (coinCount < 50 && coinCount >= 40)
            {
                SceneManager.LoadScene("A3WinScreen");
            }
            else if (coinCount < 40 && coinCount >= 30)
            {
                SceneManager.LoadScene("B3WinScreen");
            }
            else if (coinCount < 30 && coinCount >= 20)
            {
                SceneManager.LoadScene("C3WinScreen");
            }
            else if (coinCount < 20 && coinCount >= 10)
            {
                SceneManager.LoadScene("D3WinScreen");
            }
            else
            {
                SceneManager.LoadScene("F3WinScreen");
            }
        }
    }
    ...
    void loadDeath()
    {
        SceneManager.LoadScene("Death3Scene");
    }
