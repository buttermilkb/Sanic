...
public class PlayerMove : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D collider)
    {
        ...
        if (collider.gameObject.CompareTag("Win"))
        {
            if (coinCount == 50)
            {
                SceneManager.LoadScene("SWinScene");
            }
            else if (coinCount < 50 && coinCount >= 40)
            {
                SceneManager.LoadScene("AWinScene");
            }
            else if (coinCount < 40 && coinCount >= 30)
            {
                SceneManager.LoadScene("BWinScene");
            }
            else if (coinCount < 30 && coinCount >= 20)
            {
                SceneManager.LoadScene("CWinScene");
            }
            else if (coinCount < 20 && coinCount >= 10)
            {
                SceneManager.LoadScene("DWinScene");
            }
            else
            {
                SceneManager.LoadScene("FWinScene");
            }
        }
    }
